...
class APITester:
    ...
    def test_search_endpoint(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test("/api/v1/search", "POST", "PASS", 
                            f"Found {total_results} players")
                
                # Return first player ID for other tests
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log_test("/api/v1/search", "POST", "PASS", 
                            f"Query search found {data.get('total', 0)} players")
            else:
                self.log_test("/api/v1/search", "POST", "FAIL", 
                            f"Query search failed: {response.status_code}")
        except Exception as e:
            self.log_test("/api/v1/search", "POST", "FAIL", f"Query search error: {str(e)}")
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                self.log_test("/api/v1/search", "POST", "PASS", 
                            f"Filtered search found {data.get('total', 0)} players")
            else:
                self.log_test("/api/v1/search", "POST", "FAIL", 
                            f"Filtered search failed: {response.status_code}")
        except Exception as e:
            self.log_test("/api/v1/search", "POST", "FAIL", f"Filtered search error: {str(e)}")
        
        return None
    
    def test_recommendations_endpoint(self, player_id: str):
        """Test the recommendations endpoint"""
        print("\n=== Testing Recommendations Endpoint ===")
        
        if not player_id:
            self.log_test("/api/v1/recommendations", "GET", "SKIP", "No player ID available")
            return
    ...
    def run_all_tests(self):
        ...
        # Test search endpoint and get a player ID
        player_id = self.test_search_endpoint()
        
        # Test recommendations endpoint
        self.test_recommendations_endpoint(player_id)
